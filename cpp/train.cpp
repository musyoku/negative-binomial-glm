...
int main(int argc, char *argv[]){
	...
	while(true){
		...
		if(itr % 1000 == 0){
			cout << "itr: " << itr << " - log likelihood: " << trainer->compute_joint_log_likelihood() << " - MCMC acceptance: " << trainer->get_acceptance_rate() << " - precision: " << trainer->compute_mean_precision(0.99, 15) << endl;
		}
		if(itr % 10000 == 0){
